<template>
	<div>
		...
		<div class="listWrap">
			<table class="tbList">
				...
				<tr v-for="(row, idx) in list" :key="idx">
					<td>{{row.id}}</td>
					<td class="txt_left"><a href="javascript:;" @click="fnView(`${row.id}`)" >{{row.title}}</a></td>
					<td>{{row.name}}</td>
					<td>{{row.status}}</td>
					<td>{{row.rgstdate.substring(0,10)}}</td>
				</tr>
		...
		<div class="btnRightWrap">
			<a @click="fnAdd" class="btn">결재 작성</a>
		</div>
	</div>
</template>
...
export default {
	...
	, methods:{
		fnGetList() { //데이터 가져오기 함수
			...
			this.$axios.get('http://localhost:8001/list',{params:this.body})
			.then((res)=>{
		...
		,getList() {
			this.$axios.get("http://localhost:8001/list")
			.then((res)=>{
				console.log(res);
			})
			.then((err)=>{
        ...
        , fnView(id) {//조회 페이지 이동
            this.body.id = id;
                this.$router.push({path:'./read',query:this.body}); //추가한 상세페이지 라우터
		}
	}
}
</script>
